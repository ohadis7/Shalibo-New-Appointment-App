...
<head>
    ...
    <meta property="og:title" content="<?= lang('page_title') . ' ' . e(vars('company_name')) ?> | Shalibo Wellness"/>
    ...
    <title><?= lang('page_title') . ' ' . e(vars('company_name')) ?> | Shalibo Wellness</title>

    <link rel="icon" type="image/x-icon" href="<?= asset_url('assets/img/favicon.ico') ?>">
    <link rel="icon" sizes="192x192" href="<?= asset_url('assets/img/shalibo-logo.png') ?>">
    ...
    <!-- Shalibo Wellness: Redirect to landing page if no service/provider/services in URL -->
    <script>
    (function() {
        var p = new URLSearchParams(location.search);
        if (!p.get('service') && !p.get('provider') && !p.get('services')) {
            location.replace('/index.php/landing');
        }
    })();
    </script>
...
<!-- Shalibo Wellness: Custom booking enhancer (inlined) -->
<script>
(function () {
  'use strict';

  /* Use the Shalibo PNG logo for the booking header */
  var LOGO_IMG = '<img src="' + window.location.origin + '/assets/img/shalibo-logo.png" alt="Shalibo Wellness" style="width:72px;height:72px;border-radius:50%;object-fit:cover;">';
  ...
  function filterServicesByProvider() {
    ...
    if (typeof window.vars === 'function') {
      ...
      if (provider && provider.services && provider.services.length > 0) {
        filterServiceOptions(provider.services.map(String));
      }
    }
  }
